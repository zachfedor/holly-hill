...
function lcm_search_form() {
    ...
    if ( genesis_html5() )
        $form = sprintf( '<form method="get" class="search-form close" action="%s" role="search">%s<input type="search" name="s" %s="%s" /><input type="submit" value="%s" /></form>', home_url( '/' ), esc_html( $label ), $value_or_placeholder, esc_attr( $search_text ), esc_attr( $button_text ) );
    else
